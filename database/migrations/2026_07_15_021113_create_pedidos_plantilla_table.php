...
return new class extends Migration
{
    ...
    {
        Schema::create('pedidos_plantilla', function (Blueprint $table) {
            $table->id();
            $table->foreignId('cliente_id')->constrained('clientes')->onDelete('cascade');
            $table->foreignId('pedido_maestro_id')->nullable()
                  ->constrained('pedidos_maestro')->nullOnDelete();
            $table->string('codigo', 20)->unique();       // ROP-2026-001
    ...
    {
        Schema::dropIfExists('pedidos_plantilla');
    }
};
