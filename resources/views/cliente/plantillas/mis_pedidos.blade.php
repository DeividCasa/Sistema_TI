...
@section('titulo', 'Mis pedidos de ropa')
...
<div class="sec-header reveal" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:15px;margin-bottom:20px;">
  <div class="sec-title">Mis pedidos de ropa</div>
  <a href="{{ route('cliente.catalogo.index') }}" class="btn-primary" style="text-decoration:none;">+ Nuevo pedido</a>
</div>
  ...
  <div class="card reveal" style="margin-bottom:18px;overflow:hidden;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 20px;background:var(--bg-3);border-bottom:1px solid var(--border);flex-wrap:wrap;gap:10px;">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
        ...
        @endphp
        <span style="background:{{ $bg }};color:{{ $color }};padding:4px 12px;border-radius:6px;font-size:0.75rem;font-weight:600;">{{ $texto }}</span>
      </div>
    </div>
    ...
    <div style="padding:14px 20px;">
      @foreach($pedido->items as $item)
        <div style="display:flex;align-items:center;gap:14px;padding:8px 0;border-bottom:1px dashed var(--border);">
          @if($item->plantilla && $item->plantilla->imagen_preview)
            <img src="{{ asset('storage/' . $item->plantilla->imagen_preview) }}"
                 style="width:44px;height:44px;object-fit:cover;border-radius:8px;border:1px solid var(--border);">
          @else
            <div style="width:44px;height:44px;border-radius:8px;border:1px solid var(--border);background:var(--bg-3);"></div>
          @endif

          <div style="flex:1;font-size:0.85rem;">
            <div style="font-weight:600;color:var(--text-1);">{{ $item->plantilla->nombre ?? 'Producto eliminado' }}</div>
            <div style="color:var(--text-3);font-size:0.76rem;">
              @if($item->talla) Talla {{ $item->talla }} × @endif{{ $item->cantidad }}
              — ${{ number_format($item->precio_unitario, 2) }} c/u
            </div>
          </div>

          <strong style="color:var(--text-1);font-size:0.88rem;">${{ number_format($item->subtotal, 2) }}</strong>
        </div>
      @endforeach
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;flex-wrap:wrap;gap:10px;">
        <div style="font-size:0.85rem;color:var(--text-2);">
          ...
          <span style="margin-left:12px;">Adelanto (50%): <strong style="color:var(--blue);">${{ number_format($pedido->precio_adelanto, 2) }}</strong></span>
        </div>

        @if($pedido->estado_pago !== 'pagado_completo')
          <a href="{{ route('cliente.plantillas.pago', $pedido->id) }}" class="btn-primary" style="text-decoration:none;padding:9px 20px;font-size:0.85rem;">
            Ir a pagos
          </a>
        @endif
      </div>
