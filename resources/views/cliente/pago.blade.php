...
<div class="sec-header reveal">
  ...
  <a href="{{ route('cliente.uniformes.mis-pedidos') }}" class="btn-secondary">← Mis pedidos</a>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1.2fr;gap:24px;align-items:start;">
  ...
  <div class="card card-pad reveal">
    ...
    @foreach($pedido->items as $item)
      <div style="display:flex;justify-content:space-between;font-size:0.85rem;color:var(--text-2);padding:6px 0;border-bottom:1px dashed var(--border);">
        <span>{{ $item->uniforme->nombre }} — Talla {{ $item->talla }} × {{ $item->cantidad }}</span>
  ...
  <div class="card card-pad reveal">
    ...
    @else
      ...
      <form action="{{ route('cliente.uniformes.comprobante', $pedido->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <label for="archivo-uniforme" id="drop-area-uniforme" style="display:flex;flex-direction:column;align-items:center;justify-content:center;
          ...
          <input type="file" id="archivo-uniforme" name="archivo" accept="image/*,.pdf"
            onchange="previsualizarArchivo(this, 'preview-archivo-uniforme', 'drop-area-uniforme')" style="display:none;" required>
        </label>
        <div id="preview-archivo-uniforme" style="display:none;margin-top:12px;margin-bottom:16px;"></div>
