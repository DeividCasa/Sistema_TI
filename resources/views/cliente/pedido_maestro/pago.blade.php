...
<div class="sec-header reveal">
  <div class="sec-title">Pago del pedido {{ $pedido->codigo }} <span style="font-size:0.7rem;font-weight:700;color:var(--blue);background:var(--blue-soft);padding:3px 10px;border-radius:6px;text-transform:uppercase;margin-left:8px;">{{ $etiquetaTipo }}</span></div>
  <a href="{{ route('cliente.mis-pedidos') }}" class="btn-secondary">← Mis pedidos</a>
</div>

<div class="grid-2col" style="--cols:1fr 1.2fr;gap:24px;align-items:start;">
  ...
  <div class="card card-pad reveal">
    ...
    @foreach($lineas as $linea)
      <div style="display:flex;align-items:center;gap:12px;font-size:0.85rem;color:var(--text-2);padding:8px 0;border-bottom:1px dashed var(--border);">
        @if($linea['imagen'])
        ...
        @endif
        <span style="flex:1;">{{ $linea['nombre'] }} — {{ $linea['detalle'] }}</span>
        <strong style="color:var(--text-1);">${{ number_format($linea['subtotal'], 2) }}</strong>
      </div>
    @endforeach
  ...
  <div class="card card-pad reveal">
    ...
    @else
      ...
      <form action="{{ $rutaComprobante }}" method="POST" enctype="multipart/form-data">
        ...
        <label for="archivo-pago" id="drop-area-pago" style="display:flex;flex-direction:column;align-items:center;justify-content:center;
          ...
          <input type="file" id="archivo-pago" name="archivo" accept="image/*,.pdf"
            onchange="previsualizarArchivo(this, 'preview-archivo-pago', 'drop-area-pago')" style="display:none;" required>
        </label>
        <div id="preview-archivo-pago" style="display:none;margin-top:12px;margin-bottom:16px;"></div>
