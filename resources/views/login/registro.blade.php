...
<script>
  ...
  campos.cedula.addEventListener('blur', function () {
    ...
    fetch(@json(route('registro.consultar-cedula')), {
      ...
      body: JSON.stringify({ cedula: campos.cedula.value }),
    })
      .then(res => res.ok ? res.json() : Promise.reject())
      .then(data => {
        if (data.encontrado) {
          if (!campos.nombre.value.trim()) campos.nombre.value = data.nombres;
          if (!campos.apellido.value.trim()) campos.apellido.value = data.apellidos;
