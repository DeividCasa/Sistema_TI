...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start;">
  ...
  <div class="card card-pad reveal">
    ...
    @else
    <form action="{{ route('admin.pedidos-plantillas.update', $pedido->id) }}" method="POST">
      @csrf
      @method('PUT')
  ...
  <table style="width:100%;border-collapse:collapse;font-size:0.87rem;">
    <thead>
      <tr style="background:var(--bg-3);text-align:left;">
        ...
        <th style="padding:12px 16px;color:var(--text-2);font-weight:600;">Tipo</th>
        <th style="padding:12px 16px;color:var(--text-2);font-weight:600;">Talla</th>
        <th style="padding:12px 16px;color:var(--text-2);font-weight:600;">Color</th>
        ...
        <tr style="border-top:1px solid var(--border);">
          <td style="padding:10px 16px;">
            @if($item->plantilla && $item->plantilla->imagen_preview)
              <img src="{{ asset('storage/' . $item->plantilla->imagen_preview) }}" alt="" onclick="abrirLightbox(this.src)"
          ...
          <td style="padding:10px 16px;color:var(--text-2);text-transform:capitalize;">{{ $item->plantilla->tipo_prenda ?? '—' }}</td>
          <td style="padding:10px 16px;">
            @if($item->talla)
          ...
          <td style="padding:10px 16px;">
            @if($item->color)
              <span style="width:18px;height:18px;border-radius:50%;background:{{ $item->color }};border:1px solid var(--border-2);display:inline-block;"></span>
            @else — @endif
          </td>
