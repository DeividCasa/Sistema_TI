    ...
    <tbody>
      @forelse($pedidos as $pedido)
        <tr style="border-top:1px solid var(--border);">
          ...
          <td style="padding:10px 16px;color:var(--text-2);">{{ $pedido->items->count() }} chompa(s) / {{ $pedido->cantidad_total }} prenda(s)</td>
          ...
          <td style="padding:10px 16px;">
            <a href="{{ route('admin.pedidos-chompas.show', $pedido->id) }}"
               style="color:var(--blue);text-decoration:none;font-weight:600;font-size:0.82rem;">Ver detalle →</a>
          </td>
        </tr>
      @empty
        <tr>
          <td colspan="8" style="padding:32px;text-align:center;color:var(--text-3);">Aún no hay pedidos de chompas.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
