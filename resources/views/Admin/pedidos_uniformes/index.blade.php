    ...
    <tbody>
      @forelse($pedidos as $pedido)
        <tr>
          ...
          <td>{{ $pedido->items->count() }} uniforme(s) / {{ $pedido->cantidad_total }} prenda(s)</td>
          ...
          <td>
            ...
            @if($pedido->estado_pago !== 'pagado_completo')
              <form action="{{ route('admin.pedidos-uniformes.pago-completo', $pedido->id) }}" method="POST" style="display:inline-block;margin-left:6px;" onsubmit="return confirm('¿Marcar este pedido como pagado por completo?');">
                @csrf
          ...
          <td class="cell-actions">
            <a href="{{ route('admin.pedidos-uniformes.show', $pedido->id) }}">Ver detalle</a>
          </td>
        </tr>
      @empty
        <tr>
          <td colspan="8" class="cell-empty">Aún no hay pedidos de uniformes.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
