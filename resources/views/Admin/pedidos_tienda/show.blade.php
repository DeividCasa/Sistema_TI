...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start;">
  ...
  {{-- RESUMEN DE PAGO --}}
  <div class="card card-pad reveal">
    <div style="font-size:1rem;font-weight:700;color:var(--text-1);margin-bottom:14px;">Resumen de pago (combinado)</div>
    ...
    <div style="font-size:0.82rem;color:var(--text-2);line-height:1.9;">
      @if($pedido->pedidoPlantilla)
        <div style="display:flex;justify-content:space-between;"><span>Adelanto ropa:</span> <strong>${{ number_format($pedido->pedidoPlantilla->precio_adelanto, 2) }}</strong></div>
      @endif
      @if($pedido->pedidoUniforme)
        <div style="display:flex;justify-content:space-between;"><span>Adelanto uniformes:</span> <strong>${{ number_format($pedido->pedidoUniforme->precio_adelanto, 2) }}</strong></div>
      @endif
      @if($pedido->pedidoChompa)
        <div style="display:flex;justify-content:space-between;"><span>Adelanto chompas:</span> <strong>${{ number_format($pedido->pedidoChompa->precio_adelanto, 2) }}</strong></div>
      @endif
    </div>
  </div>
</div>

{{-- ESTADO DE CADA SUB-PEDIDO --}}
        ...
        <div style="display:flex;gap:10px;">
          <select name="estado"
            ...
            @foreach(['recibido','en_produccion','listo','enviado','entregado','cancelado'] as $estado)
              <option value="{{ $estado }}" {{ $pedido->pedidoPlantilla->estado === $estado ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ',$estado)) }}</option>
            @endforeach
          </select>
      ...
      <form action="{{ route('admin.pedidos-uniformes.update', $pedido->pedidoUniforme->id) }}" method="POST">
        ...
        <div style="display:flex;gap:10px;">
          <select name="estado"
            style="flex:1;padding:10px 12px;border:1.5px solid var(--border);border-radius:8px;font-family:var(--font-b);font-size:0.9rem;background:var(--bg-2);color:var(--text-1);">
            @foreach(['recibido','en_produccion','listo','enviado','entregado','cancelado'] as $estado)
              <option value="{{ $estado }}" {{ $pedido->pedidoUniforme->estado === $estado ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ',$estado)) }}</option>
            @endforeach
          </select>
          <button type="submit" class="btn-primary" style="padding:10px 18px;">Actualizar</button>
        </div>
      ...
      <form action="{{ route('admin.pedidos-chompas.update', $pedido->pedidoChompa->id) }}" method="POST">
        ...
        <div style="display:flex;gap:10px;">
          <select name="estado"
            style="flex:1;padding:10px 12px;border:1.5px solid var(--border);border-radius:8px;font-family:var(--font-b);font-size:0.9rem;background:var(--bg-2);color:var(--text-1);">
            @foreach(['recibido','en_produccion','listo','enviado','entregado','cancelado'] as $estado)
              <option value="{{ $estado }}" {{ $pedido->pedidoChompa->estado === $estado ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ',$estado)) }}</option>
            @endforeach
          </select>
          <button type="submit" class="btn-primary" style="padding:10px 18px;">Actualizar</button>
        </div>
