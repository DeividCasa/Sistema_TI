...
@php
  ...
  $todosVerificados = $hijosPedido->every(fn ($h) => \App\Support\PedidoEstados::pagoVerificado($h->estado_pago));
@endphp

{{-- Columna izquierda: datos del cliente + comprobantes que subió.
     Columna derecha: resumen de pago y el control de estado, todo en una
     sola tarjeta (evita repetir el mismo "cuadro" para dos cosas que
     siempre se miran juntas al decidir si se puede avanzar el pedido). --}}
<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start;">
  ...
  <div style="display:flex;flex-direction:column;gap:24px;">
    ...
    <div class="card reveal" style="overflow:hidden;">
      ...
      @forelse($pedido->comprobantes as $comprobante)
        <div style="display:flex;gap:16px;padding:16px 20px;border-top:1px solid var(--border);align-items:flex-start;flex-wrap:wrap;">
          @if(Str::endsWith(strtolower($comprobante->archivo), '.pdf'))
            <a href="{{ asset('storage/' . $comprobante->archivo) }}" target="_blank">
              <div style="width:96px;height:96px;border-radius:10px;background:var(--bg-3);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;">
          ...
          @else
            <img src="{{ asset('storage/' . $comprobante->archivo) }}" alt="Voucher" onclick="abrirLightbox(this.src)"
                 style="width:96px;height:96px;object-fit:cover;border-radius:10px;border:1px solid var(--border);cursor:zoom-in;">
          @endif

          <div style="flex:1;min-width:180px;font-size:0.85rem;color:var(--text-2);line-height:1.8;">
            <div>
  ...
  {{-- RESUMEN DE PAGO + ESTADO DE PRODUCCIÓN GENERAL, en una sola tarjeta
       (un solo control para todo el pedido: ropa, uniformes y chompas
       avanzan juntos, con un solo aviso al cliente) --}}
  <div class="card card-pad reveal">
    ...
    @if(!$todosVerificados)
      <div style="background:#FEF3C7;border:1px solid #FDE68A;color:#92400E;padding:12px 14px;border-radius:10px;font-size:0.82rem;line-height:1.5;">
        🔒 Verifica todos los comprobantes de pago del pedido antes de cambiar el estado.
      </div>
    @else
    <form action="{{ route('admin.pedidos-tienda.actualizar-estado', $pedido->id) }}" method="POST">
      @csrf
      @method('PUT')
      ...
      <select name="estado"
        ...
        @foreach(\App\Support\PedidoEstados::estadosDisponibles($pedido->tipo_entrega) as $estado)
          <option value="{{ $estado }}" {{ $estadoActual === $estado ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ',$estado)) }}</option>
        @endforeach
      </select>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:0.87rem;">
      <thead>
        <tr style="background:var(--bg-3);text-align:left;">
          ...
          <th style="padding:12px 16px;color:var(--text-2);font-weight:600;">Producto</th>
          ...
          <tr style="border-top:1px solid var(--border);">
            <td style="padding:10px 16px;">
              @if($item->plantilla && $item->plantilla->imagen_preview)
                <img src="{{ asset('storage/' . $item->plantilla->imagen_preview) }}" alt="" onclick="abrirLightbox(this.src)" style="width:64px;height:64px;object-fit:cover;border-radius:8px;border:1px solid var(--border);cursor:zoom-in;">
              @endif
            </td>
            <td style="padding:10px 16px;font-weight:600;color:var(--text-1);">{{ $item->plantilla->nombre ?? 'Producto eliminado' }}</td>
            <td style="padding:10px 16px;">
              @if($item->talla)
    ...
    <table style="width:100%;border-collapse:collapse;font-size:0.87rem;">
      ...
      <tbody>
        @foreach($pedido->pedidoUniforme->items as $item)
          <tr style="border-top:1px solid var(--border);">
            <td style="padding:10px 16px;">
              <img src="{{ asset('storage/' . $item->uniforme->imagen) }}" alt="" onclick="abrirLightbox(this.src)" style="width:64px;height:64px;object-fit:cover;border-radius:8px;border:1px solid var(--border);cursor:zoom-in;">
            </td>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:0.87rem;">
      ...
      <tbody>
        @foreach($pedido->pedidoChompa->items as $item)
          <tr style="border-top:1px solid var(--border);">
            <td style="padding:10px 16px;">
              <img src="{{ asset('storage/' . $item->chompa->imagen) }}" alt="" onclick="abrirLightbox(this.src)" style="width:64px;height:64px;object-fit:cover;border-radius:8px;border:1px solid var(--border);cursor:zoom-in;">
            </td>
